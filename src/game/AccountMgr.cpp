...
AccountOpResult AccountMgr::CreateAccount(std::string username, std::string password)
{
    ...
    LoginDatabase.Execute("INSERT INTO realmcharacters (realmid, acctid, numchars) SELECT realmlist_classic.id, account.id, 0 FROM realmlist_classic,account LEFT JOIN realmcharacters ON acctid=account.id WHERE acctid IS NULL");
