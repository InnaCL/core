...
bool ChatHandler::HandleNameWannounceCommand(char* args)
{
    WorldPacket data;
    if(!*args)
        return false;
	...
	QueryResult* Seleccionar_Nivel_Cuenta = LoginDatabase.PQuery("SELECT gmlevel FROM account WHERE id = '%u'", Seleccionar_Id_Cuenta_valor);
	Field* fields2 = Seleccionar_Nivel_Cuenta->Fetch();
	uint32 Seleccionar_Nivel_Cuenta_valor = fields2[0].GetUInt32();
	
	char* gm;
	switch (GetSession()->GetSessionDbcLocale())
	{
		if (Seleccionar_Nivel_Cuenta_valor == 1)
	case LOCALE_esES: gm = "Moderador"; break;
		if (Seleccionar_Nivel_Cuenta_valor == 1)
	case LOCALE_enUS: gm = "Moderator"; break;
		if (Seleccionar_Nivel_Cuenta_valor == 1)
	default:  gm = "Moderator";
	}
	switch (GetSession()->GetSessionDbcLocale())
	{

		if (Seleccionar_Nivel_Cuenta_valor == 2)
	case LOCALE_esES: gm = "Maestro de Consultas"; break;
		if (Seleccionar_Nivel_Cuenta_valor == 2)
	case LOCALE_enUS:	gm = "Ticket Master"; break;
		 if (Seleccionar_Nivel_Cuenta_valor == 2)
	default:	gm = "Ticket Master"; break;
	}
	switch (GetSession()->GetSessionDbcLocale())
	{
		if (Seleccionar_Nivel_Cuenta_valor == 3)
	case LOCALE_esES:	gm = "Maestro del Juego"; break;
		if (Seleccionar_Nivel_Cuenta_valor == 3)
	case LOCALE_enUS:		gm = "Game Master"; break;
		 if (Seleccionar_Nivel_Cuenta_valor == 3)
	default:	gm = "Game Master"; break;
	}
	switch (GetSession()->GetSessionDbcLocale())
	{
		if (Seleccionar_Nivel_Cuenta_valor == 4)
	case LOCALE_esES:	gm = "Admin Basico"; break;
		if (Seleccionar_Nivel_Cuenta_valor == 4)
	case LOCALE_enUS:	gm = "Basic Admin"; break;
		 if (Seleccionar_Nivel_Cuenta_valor == 4)
	default:	gm = "Basic Admin"; break;
	}
	switch (GetSession()->GetSessionDbcLocale())
	{
		if (Seleccionar_Nivel_Cuenta_valor == 5)
	case LOCALE_esES:	gm = "Desarollador"; break;
		if (Seleccionar_Nivel_Cuenta_valor == 5)
	case LOCALE_enUS:	gm = "Developer"; break;
		 if (Seleccionar_Nivel_Cuenta_valor == 5)
	default:	gm = "Developer"; break;
	}
	switch (GetSession()->GetSessionDbcLocale())
	{
		if (Seleccionar_Nivel_Cuenta_valor == 6)
	case LOCALE_esES:	gm = "Administrador"; break;
		if (Seleccionar_Nivel_Cuenta_valor == 6)
	case LOCALE_enUS:	gm = "Administrator"; break;
		 if (Seleccionar_Nivel_Cuenta_valor == 6)
	default:	gm = "Administrator"; break;
	}

    sWorld.SendAllWorldText("|cffffff00[|r|cffffffffIron WoW|r|cffffff00]|r |cff6cb6f7<%s>|r |cffFF0000%s:|r |cffffff00%s", gm, name.c_str(), args);
    return true;
	}
...
bool ChatHandler::HandleReloadGMRanks(char*)
{
	sAccountMgr.Load();
	SendSysMessage(">> Table `GM Ranks` reloaded.");
	return true;
}
