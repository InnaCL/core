...
void RealmList::UpdateRealm( uint32 ID, const std::string& name, const std::string& address, uint32 port, uint8 icon, Flags flag, uint8 timezone, AccountTypes allowedSecurityLevel, float popu, const std::string& builds)
{
...
void RealmList::UpdateRealms(bool init)
{
    ...
    if(result)
    {
        do
        {
            ...
            if (flag & ~(REALM_FLAG_OFFLINE|REALM_FLAG_NEW_PLAYERS|REALM_FLAG_RECOMMENDED|REALM_FLAG_SPECIFYBUILD))
            {
				...
				flag &= (REALM_FLAG_OFFLINE|REALM_FLAG_NEW_PLAYERS|REALM_FLAG_RECOMMENDED|REALM_FLAG_SPECIFYBUILD);
            }

            UpdateRealm(
                fields[0].GetUInt32(), fields[1].GetCppString(),fields[2].GetCppString(),fields[3].GetUInt32(),
                fields[4].GetUInt8(), Flags(flag), fields[6].GetUInt8(),
